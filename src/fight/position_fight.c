...
int init_position_fight(game_t *game)
{
    game->fight_status[0].position = (sfVector2f){120, -2960};
    game->fight_status[1].position = (sfVector2f){120, -2930};
...
int set_position_fight(game_t *game)
{
    ...
    for (int i = 0; i != 2; ++i)
        sfSprite_setPosition
        (game->fight_status[i].sprite, game->fight_status[i].position);
