...
int position_coin_animation(game_t *game)
{
    ...
    while (a <= 19) {
        position_sprite
        (game->anime[a].sprite, &game->anime[a].position, 0, x_a);
        x_a += 160;
        a += 2;
        position_sprite
        (game->anime[b].sprite, &game->anime[b].position, 0, x_b);
        x_b += 160;
        b += 2;
    }
    return 0;
...
static int anime_the_coin(game_t *game)
{
    ...
    for (int i = 0; i <= 100; i++) {
        ...
        (sfVector2f){game->anime[a].position.x, game->anime[a].position.y + 1};
        ...
        (sfVector2f){game->anime[b].position.x, game->anime[b].position.y + 1};
...
static int set_pos_coin(game_t *game)
{
    int x_a = -1200;
    int a = 0;
    int b = 1;
    ...
    while (a <= 19) {
        position_sprite
        (game->anime[a].sprite, &game->anime[a].position,
        game->player.position.y - 1500, x_a);
        x_a += 120;
        a += 2;
        position_sprite
        (game->anime[b].sprite, &game->anime[b].position,
        game->player.position.y - 1500, x_a);
        x_a += 120;
        b += 2;
    }
